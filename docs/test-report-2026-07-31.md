...
**分支**: main  
**提交**: 0e99f64  
**结果**: 全部通过 (88 passed / 0 failed / 0 ignored)
...
| converter::test_update_event_splits_before_after | passed |
...
| 数据模型 | canal-common | 11 | 类型、序列化、位点排序 |
| 数据流 | canal-binlog | 8 | binlog 转换、表映射 |
| 存储层 | canal-store | 8 | 内存缓冲、位点追踪、溢出 |
| 过滤 | canal-filter | 6 | 正则匹配、黑名单 |
| 实例管理 | canal-instance | 6 | 注册、查询、生命周期 |
| 元数据 | canal-meta | 6 | 表信息缓存、主键 |
| 监控 | canal-prometheus | 5 | 计数器、Gauge、HTTP 端点 |
| 连接器 | canal-connector | 4 | Kafka 序列化 |
| API | canal-admin | 3 | REST 健康检查、实例管理 |
...
- **二进制协议覆盖完整**: canal-server 的 25 个测试覆盖了编解码边界情况（不完整包头/负载）、协议 round-trip（Auth/Sub/Get/Ack/Rollback）以及 CanalEvent→Entry 转换的各种 DML 类型
- **存储层正确性验证**: 包含 buffer 溢出驱逐、生命周期启停、空批处理等边界场景
